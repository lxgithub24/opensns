	...
	private function filterWeibo($aType, $param) {
		...
		if ($aType == 'fav') {
			...
			$param ['where'] ['id'] = array (
					'in',
					implode ( ',', getSubByKey ( $support, 'row' ) ) 
			);
...
// 			$weiboContent = '自己写的，作为标志';
	...
	public function weiboDetail($id) {
		...
		$supported = D ( 'Weibo' )->getSupportedPeople ( $weibo ['id'], array (
				'nickname',
				'space_url',
				'avatar128',
				'space_link' 
